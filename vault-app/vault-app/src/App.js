...
function Personal({ entries, categories, onRefresh }) {
  ...
  const catTotals = categories.map(c => ({
    ...c,
    total: monthEntries.filter(e => e.category_id === c.id && e.type === 'expense').reduce((s,e) => s + Number(e.amount), 0)
  ...
  const deleteEntry = async (id) => {
    ...
    await supabase.from('personal_entries').delete().eq('id', id)
    await onRefresh()
  }
    ...
    <div className="section">
      ...
      <div className="stat-grid" style={{marginBottom:'1.5rem'}}>
        <div className="stat-card success">
          ...
          <div className="stat-val">{fmt(income)}</div>
        </div>
        <div className="stat-card danger">
